object frm_avaliacao: Tfrm_avaliacao
  Left = 0
  Top = 0
  Caption = 'Avalia'#231#227'o'
  ClientHeight = 612
  ClientWidth = 872
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 13
  object lbl_data: TLabel
    Left = 64
    Top = 48
    Width = 87
    Height = 13
    Caption = 'Data da Avalia'#231#227'o'
  end
  object lbl_aluno: TLabel
    Left = 64
    Top = 93
    Width = 75
    Height = 13
    Caption = 'Escolha o Aluno'
  end
  object lbl_queixa: TLabel
    Left = 329
    Top = 45
    Width = 84
    Height = 13
    Caption = 'Queixas do Aluno'
    FocusControl = mm_queixa
  end
  object Label4: TLabel
    Left = 64
    Top = 135
    Width = 36
    Height = 13
    Caption = 'H'#225'bitos'
    FocusControl = mm_habitos
  end
  object lbl_doenca: TLabel
    Left = 64
    Top = 215
    Width = 41
    Height = 13
    Caption = 'Doen'#231'as'
    FocusControl = mm_doenca
  end
  object lbl_antecedentes: TLabel
    Left = 329
    Top = 210
    Width = 67
    Height = 13
    Caption = 'Antecedentes'
    FocusControl = mm_antecedentes
  end
  object lbl_tratamento: TLabel
    Left = 64
    Top = 290
    Width = 59
    Height = 13
    Caption = 'Tratamento '
    FocusControl = mm_tratamento
  end
  object lbl_dor_mov: TLabel
    Left = 329
    Top = 290
    Width = 98
    Height = 13
    Caption = 'Dores de Movimento'
    FocusControl = edt_dor_mov
  end
  object lbl_cirurgia: TLabel
    Left = 592
    Top = 212
    Width = 45
    Height = 13
    Caption = 'Cirurgias '
    FocusControl = edt_cirurgias
  end
  object lbl_posicoes: TLabel
    Left = 329
    Top = 123
    Width = 102
    Height = 13
    Caption = 'Posi'#231#245'es do dia a dia '
    FocusControl = mm_posicoes
  end
  object lbl_exer_ativ: TLabel
    Left = 592
    Top = 48
    Width = 170
    Height = 13
    Caption = 'Exercicios ou atividades frequentes'
    FocusControl = edt_exer_ativ
  end
  object lbl_objetivos: TLabel
    Left = 592
    Top = 123
    Width = 91
    Height = 13
    Caption = 'Objetivos do Aluno'
    FocusControl = edt_objetivo
  end
  object btn_cancelar: TSpeedButton
    Left = 427
    Top = 428
    Width = 169
    Height = 38
    Action = DatasetCancel1
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object btn_editar: TSpeedButton
    Left = 336
    Top = 384
    Width = 169
    Height = 38
    Action = DatasetEdit1
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object btn_inserir: TSpeedButton
    Left = 136
    Top = 384
    Width = 169
    Height = 38
    Action = DatasetInsert1
    Glyph.Data = {
      
      E8E8FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object btn_salvar: TSpeedButton
    Left = 244
    Top = 428
    Width = 169
    Height = 38
    Action = DatasetPost1
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
  end
  object btn_deletar: TSpeedButton
    Left = 528
    Top = 384
    Width = 169
    Height = 38
    Hint = 'Delete'
    Caption = 'Deletar'
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    NumGlyphs = 2
    OnClick = btn_deletarClick
  end
  object calendario: TMonthCalendar
    Left = 175
    Top = 93
    Width = 194
    Height = 158
    Cursor = crHandPoint
    Date = 43999.000000000000000000
    MinDate = 40179.000000000000000000
    TabOrder = 14
    Visible = False
    OnClick = calendarioClick
  end
  object mm_queixa: TDBMemo
    Left = 328
    Top = 64
    Width = 177
    Height = 53
    DataField = 'queixa'
    DataSource = dtm.ds_avaliacao
    TabOrder = 6
  end
  object mm_habitos: TDBMemo
    Left = 64
    Top = 156
    Width = 177
    Height = 53
    DataField = 'habitos'
    DataSource = dtm.ds_avaliacao
    TabOrder = 3
  end
  object mm_doenca: TDBMemo
    Left = 64
    Top = 234
    Width = 177
    Height = 53
    DataField = 'doenca'
    DataSource = dtm.ds_avaliacao
    TabOrder = 4
  end
  object mm_antecedentes: TDBMemo
    Left = 328
    Top = 231
    Width = 177
    Height = 53
    DataField = 'antecedentes'
    DataSource = dtm.ds_avaliacao
    TabOrder = 8
  end
  object mm_tratamento: TDBMemo
    Left = 64
    Top = 309
    Width = 177
    Height = 53
    DataField = 'tratamento'
    DataSource = dtm.ds_avaliacao
    TabOrder = 5
  end
  object edt_dor_mov: TDBEdit
    Left = 328
    Top = 309
    Width = 177
    Height = 21
    DataField = 'dor_mov'
    DataSource = dtm.ds_avaliacao
    TabOrder = 9
  end
  object edt_cirurgias: TDBEdit
    Left = 592
    Top = 231
    Width = 177
    Height = 21
    DataField = 'cirurgias'
    DataSource = dtm.ds_avaliacao
    TabOrder = 12
  end
  object mm_posicoes: TDBMemo
    Left = 328
    Top = 142
    Width = 177
    Height = 53
    DataField = 'posicoes'
    DataSource = dtm.ds_avaliacao
    TabOrder = 7
  end
  object edt_exer_ativ: TDBEdit
    Left = 592
    Top = 67
    Width = 177
    Height = 21
    DataField = 'exer_ativi'
    DataSource = dtm.ds_avaliacao
    TabOrder = 10
  end
  object edt_objetivo: TDBEdit
    Left = 592
    Top = 142
    Width = 177
    Height = 21
    DataField = 'objetivos'
    DataSource = dtm.ds_avaliacao
    TabOrder = 11
  end
  object btn_calendario: TButton
    Left = 175
    Top = 62
    Width = 66
    Height = 25
    Caption = 'Calend'#225'rio'
    TabOrder = 1
    OnClick = btn_calendarioClick
  end
  object Escolher_aluno: TDBLookupComboBox
    Left = 64
    Top = 108
    Width = 177
    Height = 21
    DataField = 'aluno'
    DataSource = dtm.ds_avaliacao
    KeyField = 'id_aluno'
    ListField = 'nome_aluno'
    ListSource = dtm.ds_alunos
    TabOrder = 2
  end
  object DBGrid1: TDBGrid
    Left = 260
    Top = 484
    Width = 325
    Height = 120
    DataSource = dtm.ds_avaliacao
    Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
    TabOrder = 13
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    Columns = <
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'idavaliacao'
        Title.Alignment = taCenter
        Title.Caption = 'C'#243'digo da Avalia'#231#227'o'
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'data_avaliacao'
        Title.Alignment = taCenter
        Title.Caption = 'Data da Avalia'#231#227'o'
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'new_aluno'
        Title.Alignment = taCenter
        Title.Caption = 'Nome Aluno'
        Width = 110
        Visible = True
      end>
  end
  object edt_selecione: TDBEdit
    Left = 64
    Top = 66
    Width = 105
    Height = 21
    DataField = 'data_avaliacao'
    DataSource = dtm.ds_avaliacao
    TabOrder = 0
  end
  object ActionManager1: TActionManager
    Top = 48
    StyleName = 'Platform Default'
    object DatasetInsert1: TDataSetInsert
      Category = 'Dataset'
      Caption = 'Inserir'
      Hint = 'Insert'
      ImageIndex = 4
      DataSource = dtm.ds_avaliacao
    end
    object DatasetDelete1: TDataSetDelete
      Category = 'Dataset'
      Caption = 'Deletar'
      Hint = 'Delete'
      ImageIndex = 5
      DataSource = dtm.ds_avaliacao
    end
    object DatasetEdit1: TDataSetEdit
      Category = 'Dataset'
      Caption = 'Editar'
      Hint = 'Edit'
      ImageIndex = 6
      DataSource = dtm.ds_avaliacao
    end
    object DatasetPost1: TDataSetPost
      Category = 'Dataset'
      Caption = 'Salvar'
      Hint = 'Post'
      ImageIndex = 7
      DataSource = dtm.ds_avaliacao
    end
    object DatasetCancel1: TDataSetCancel
      Category = 'Dataset'
      Caption = 'Cancelar'
      Hint = 'Cancel'
      ImageIndex = 8
      DataSource = dtm.ds_avaliacao
    end
  end
end
